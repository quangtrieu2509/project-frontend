import { Breadcrumb } from "antd";
import { ROUTES, keyTypes } from "../../../constants";
import Browsing from "./Browsing";
import Selecting from "../Selecting";
import { useEffect, useState } from "react";
import NotFound from "../../../components/Static/NotFound";
import { useParams } from "react-router-dom";
import { apiCaller, locationApi } from "../../../api";
import { messages } from "../../../constants/message";
import { BreadcrumbItem } from "../../../types";
import { useDocumentTitle } from "../../../hooks";

export default function Dinings() {
  const [bcItems, setBcItems] = useState<BreadcrumbItem[]>([])
  const [coors, setCoors] = useState<number[]>([105.853333, 21.028333])
  const [has404Error, setHas404Error] = useState<boolean>(false)
  const { slug } = useParams()

  const title = bcItems.length ? `${bcItems[bcItems.length - 1].name} Dinings` : "Dinings"
  useDocumentTitle(title)

  useEffect(() => {
    const getBreadcrumbItems = async () => {
        const res = await apiCaller(
          locationApi.getBreadcrumb(slug ?? ""),
          (error) => {
            if (error.ec === messages.NOT_FOUND.ec) {
              setHas404Error(true)
            }
          }
        )
        
        if (res !== undefined) {
          setBcItems(res.data.breadcrumb)
          setCoors(res.data.coordinates)
        }
    }

    getBreadcrumbItems()
  }, [slug])

  const generateBcItems = (items: BreadcrumbItem[]) => {    
    const returnedItems: any = items.map((e) => {
      return {
        title: e.name,
        href: ROUTES.TOURISM_BASE + e.slug
      }
    })
  
    returnedItems.push({
      title: `${items[items.length - 1]?.name ?? ""} Dinings`
    })
  
    return returnedItems
  }

  return (
    <div className="tp-page dinings-page bg-color-background-primary">
      { has404Error ? <NotFound/> 
      : <div className="tp-wrapper">
        <Breadcrumb
          separator=">"
          items={generateBcItems(bcItems)}
        />
        {Boolean(bcItems.length) && <h1 className="text-center">
          {`Dinings in ${bcItems[bcItems.length - 1]?.name}`}
        </h1>}
        {
          bcItems.length >= 3
          ? <Browsing id={bcItems[bcItems.length - 1]?.id} coors={coors}/>
          : <Selecting id={bcItems[bcItems.length - 1]?.id}
              keyType={keyTypes.DINING}
            />
        }
      </div>}
    </div>
  )
}