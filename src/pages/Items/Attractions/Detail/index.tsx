...
import { ROUTES, pluralItemLabels, rateLevelArrRev } from "../../../../constants"
...
export default function Detail() {
  ...
  const { id } = useParams()
  const [item, setItem] = useState<AttractionDetail>()
  const dispatch = useDispatch()

  const title = item ? item.name : "Attraction Detail"
  ...
  const generateBCItems = (ancestors: any[]) => {
    ...
    items.push({
      title: `${ancestors[0].name} ${pluralItemLabels.ATTRACTION}`,
      href: ROUTES.TOURISM_BASE + `${ancestors[0].slug}/attractions`
    })
  ...
  return (
    <div className="tp-page attraction-detail-page bg-color-background-primary">
        ...
        <div className="bg-white rounded-lg border border-solid border-color-border-secondary mb-5">
          <div className="px-6 py-2">
            ...
            <div className="flex items-center text-sm text-color-text-secondary mb-1">
              ...
              <div className="flex">
                <div>
                  {filterFields(item.categories, attractionTypes, true).join(", ")}
                </div>
              </div>
            </div> 
