...
export default function Attractions() {
  ...
  const title = bcItems.length ? `${bcItems[bcItems.length - 1].name} Attractions` : "Attractions"
  ...
  const generateBcItems = (items: BreadcrumbItem[]) => {    
    ...
    returnedItems.push({
      title: `${items[items.length - 1]?.name ?? ""} Attractions`
    })
  ...
  return (
    <div className="tp-page attractions-page bg-color-background-primary">
      ...
      : <div className="tp-wrapper">
        ...
        {Boolean(bcItems.length) && <h1 className="text-center">
          {`Attractions in ${bcItems[bcItems.length - 1]?.name}`}
        </h1>}
        {
          bcItems.length >= 3
