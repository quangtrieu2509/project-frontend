import { Checkbox, Collapse, CollapseProps, Drawer, Rate } from "antd";
import Slider from "react-slick";
import "./index.style.scss"
import { itemTypes, activityTypes } from "../../../../constants";
import { useEffect, useState } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import ActivityOverview, { ActivityOverviewProps as Item } from "../../../../components/Item/ActivityOverview";
...
export default function Browsing(props: BrowsingProps) {
  ...
  useEffect(() => {
    const getQueriedList = async (id: string, data: any) => {
      const res = await apiCaller(
        itemApi.getQueriedItems(id, itemTypes.ACTIVITY, data)
      )
  ...
  useEffect(() => {
    const getBrowsingList = async (id: string) => {
      const res = await apiCaller(
        itemApi.getBrowsingItems(id, itemTypes.ACTIVITY)
      )
  ...
  const generatePopularList = () => {
    const [l1, l2, l3, l4] = [
      browsingList.tour, 
      browsingList["day-trip"], 
      browsingList["motor-tour"], 
      browsingList["scube-snorkel"]
    ]
    return (
      <>
      {Boolean(l1.length) && <div className="mb-8">
        <h2 className="mb-4 mt-0 font-semibold">Tours</h2>
        <Slider {...settings}
          className={generateSlickClass(l1.length, slickLimit)}
        >
          {l1.map((e: any) => (<ActivityOverview key={e.id} {...e}/>))}
        </Slider>
      </div>}
      {Boolean(l2.length) && <div className="mb-8">
        <h2 className="mb-4 mt-0 font-semibold">Day Trips</h2>
        <Slider {...settings}
          className={generateSlickClass(l2.length, slickLimit)}
        >
          {l2.map((e: any) => (<ActivityOverview key={e.id} {...e}/>))}
        </Slider>
      </div>}
      {Boolean(l3.length) && <div className="mb-8">
        <h2 className="mb-4 mt-0 font-semibold">Motor Tours</h2>
        <Slider {...settings}
          className={generateSlickClass(l3.length, slickLimit)}
        >
          {l3.map((e: any) => (<ActivityOverview key={e.id} {...e}/>))}
        </Slider>
      </div>}
      {Boolean(l4.length) && <div className="mb-8">
        <h2 className="mb-4 mt-0 font-semibold">Scuba & Snorkeling</h2>
        <Slider {...settings}
          className={generateSlickClass(l4.length, slickLimit)}
        >
          {l4.map((e: any) => (<ActivityOverview key={e.id} {...e}/>))}
        </Slider>
      </div>}
      </>
    )
